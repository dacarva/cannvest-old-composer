...
/**
 * A lot has changed its license status by the government
 * @param {org.agrotracker.network.ProductLotLicenseUpdate} lotLicenseUpdated - the ProductLotReceived transaction
 * @transaction
 */
async function changeLicense(lotLicenseUpdated) {

    // const contract = lotLicenseUpdated.lot.licenseContract;
    const lot = lotLicenseUpdated.lot;

    let licenseStatus = lotLicenseUpdated.licenseStatus;

    console.log('The license status is: ' + licenseStatus);

    // set the license status of the lot
    lot.licenseStatus = licenseStatus;
...
async function setupDemo(setupDemo) {  // eslint-disable-line no-unused-vars
    ...
    lot.unitCount = 5000;
    lot.licenseStatus = true;
    lot.contract = factory.newRelationship(NS, 'Contract', 'CON_001');
    lot.licenseContract = factory.newRelationship(NS, 'LicenseContract', 'LICON_001');
